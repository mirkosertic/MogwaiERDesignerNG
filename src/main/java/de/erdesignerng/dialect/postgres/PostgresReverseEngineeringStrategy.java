...
public class PostgresReverseEngineeringStrategy extends JDBCReverseEngineeringStrategy<PostgresDialect> {
	...
	public PostgresReverseEngineeringStrategy(final PostgresDialect aDialect) {
		super(aDialect);
	}

	@Override
	public List<SchemaEntry> getSchemaEntries(final Connection aConnection)
			throws SQLException {
	...
	// wrong
	@Override
	protected void reverseEngineerAttribute(final Attribute<Table> aAttribute, final TableEntry aTableEntry, final Connection aConnection) {
	...
	protected void reverseEngineerDomain(final Model aModel, final Domain aDomain, final ReverseEngineeringOptions aOptions, final ReverseEngineeringNotifier aNotifier, final Connection aConnection) {
		...
		final String theQuery = "SELECT t.oid, n.nspname, t.typname, format_type(t.oid, null) AS alias, d.description "
		...
		for (final SchemaEntry theEntry : aOptions.getSchemaEntries()) {
			try {
				...
				try {
					...
					if (theResult.next()) {
						final String theDescription = theResult.getString("description");
		...
		for (final SchemaEntry theEntry : aOptions.getSchemaEntries()) {
			...
			do {
				...
				try {
					...
					while (theResult.next()) {
						final String theTypeType = theResult.getString("typcategory");
						final String theSchemaName = theResult.getString("nspname");
						final String theTypeName = theResult.getString("typname");
						final String theTypeNameAlias = theResult.getString("alias");
						final String theDescription = theResult.getString("description");
...
//enumeration
							if (theTypeType.equals("E")) {
								...
								final String theAttributesQuery = "SELECT enumlabel "
										+ "FROM pg_enum "
										+ "WHERE enumtypid = ?";
								final PreparedStatement theAttributesStatement;
								...
								try {
									...
									while (theAttributesResult.next()) {
										...
										final Attribute<CustomType> theAttribute = new Attribute<>();
...
//composite
								...
								final String theAttributesQuery = "SELECT a.attname, format_type(t.oid,NULL) AS typname, a.attndims, a.atttypmod, n.nspname "
								...
								try {
									...
									while (theAttributesResult.next()) {
										...
										String theAttributeName = null;
										final Integer theTypeProperties;
										...
										final DataType theDataType = aModel.getDialect().getDataTypes().findByName(theAttributeTypeName);
										if (theDataType == null) {
										...
										try {
											...
											if (theTypeProperties > -1) {
												final int theTemp = (theTypeProperties % 65536);
												final int theSizeTemp = (theTypeProperties / 65536);
...
//								theCustomType.setType(CustomTypeType.EXTERNAL);
					...
					if ((e.getClass().getName().equals("org.postgresql.util.PSQLException")) && (thePreviousException == null || !thePreviousException.getMessage().equals(e.getMessage()))) {
						final String theMessage = e.getMessage();
						final int theFieldNameStart = theMessage.lastIndexOf(".");
						final int theFieldNameEnd = theMessage.indexOf(" ", theFieldNameStart);
						final int theExpressionStart = theMessage.substring(0, theFieldNameEnd).lastIndexOf(" ");
						final String theFieldName = theMessage.substring(theFieldNameStart + 1, theFieldNameEnd);
						final String theTarget = theMessage.substring(theExpressionStart + 1, theFieldNameEnd);
						final String theReplacement = ((theFieldName.equalsIgnoreCase("typcategory")) ? "'C'" : "NULL") + " AS " + theFieldName;
	...
	protected String reverseEngineerViewSQL(final TableEntry aViewEntry,
		...
		final PreparedStatement theStatement = aConnection.prepareStatement("SELECT * FROM information_schema.views WHERE table_name = ?");
