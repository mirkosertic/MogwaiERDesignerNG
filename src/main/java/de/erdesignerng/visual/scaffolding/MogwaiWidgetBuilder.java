...
public class MogwaiWidgetBuilder extends
		BaseWidgetBuilder<JComponent, SwingMetawidget> implements
	...
	@Override
	protected JComponent buildReadOnlyWidget(String elementName,
		...
		if (lookup != null && !"".equals(lookup)) {
			...
			if (lookupLabels != null && !"".equals(lookupLabels))
				return new LookupLabel(getLabelsMap(CollectionUtils
						.fromString(lookup), CollectionUtils
	...
	@Override
	protected JComponent buildActiveWidget(String elementName,
			Map<String, String> attributes, SwingMetawidget metawidget)
			throws Exception {
		// Hidden
		...
		if (lookup != null && !"".equals(lookup)) {
			...
			BindingConverter converter = metawidget
					.getWidgetProcessor(BindingConverter.class);
			...
			if (lookupLabels != null && !"".equals(lookupLabels)) {
				Map<String, String> labelsMap = getLabelsMap(values,
						CollectionUtils.fromString(attributes
								.get(LOOKUP_LABELS)));
		...
		if (clazz != null) {
			...
			if (clazz.isPrimitive()) {
				...
				if (minimumValue != null && !"".equals(minimumValue)
					...
					slider.setMinimum((int) Math.ceil(Double
							.parseDouble(minimumValue)));
					slider.setValue(slider.getMinimum());
					slider.setMaximum((int) Math.floor(Double
							.parseDouble(maximumValue)));
				...
				if (byte.class.equals(clazz)) {
					...
					if (minimumValue != null && !"".equals(minimumValue))
						minimum = (byte) Math.ceil(Double
								.parseDouble(minimumValue));

					if (maximumValue != null && !"".equals(maximumValue))
						maximum = (byte) Math.floor(Double
								.parseDouble(maximumValue));

					setSpinnerModel(spinner, (byte) 0, minimum, maximum,
							(byte) 1);
					...
					if (minimumValue != null && !"".equals(minimumValue))
						minimum = (short) Math.ceil(Double
								.parseDouble(minimumValue));

					if (maximumValue != null && !"".equals(maximumValue))
						maximum = (short) Math.floor(Double
								.parseDouble(maximumValue));

					setSpinnerModel(spinner, (short) 0, minimum, maximum,
							(short) 1);
					...
					if (minimumValue != null && !"".equals(minimumValue))
						minimum = (int) Math.ceil(Double
								.parseDouble(minimumValue));

					if (maximumValue != null && !"".equals(maximumValue))
						maximum = (int) Math.floor(Double
								.parseDouble(maximumValue));
					...
					if (minimumValue != null && !"".equals(minimumValue))
						minimum = (long) Math.ceil(Double
								.parseDouble(minimumValue));

					if (maximumValue != null && !"".equals(maximumValue))
						maximum = (long) Math.floor(Double
								.parseDouble(maximumValue));

					setSpinnerModel(spinner, (long) 0, minimum, maximum,
							(long) 1);
					...
					if (attributes.containsKey(MAXIMUM_FRACTIONAL_DIGITS))
						stepSize = (float) Math.pow(10, -Integer
								.parseInt(attributes
										.get(MAXIMUM_FRACTIONAL_DIGITS)));
					else
						stepSize = 0.1f;
	...
	private void setSpinnerModel(JSpinner spinner, Number value,
			Comparable<? extends Number> minimum,
	...
	private Map<String, String> getLabelsMap(List<String> values,
			List<String> labels) {
		Map<String, String> labelsMap = CollectionUtils.newHashMap();

		if (labels.size() != values.size())
			throw WidgetBuilderException
					.newException("Labels list must be same size as values list");
